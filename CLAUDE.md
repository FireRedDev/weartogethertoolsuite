...
- **Statistik lädt monatsweise und cacht je Monat** (`OrderRepository`), plus Zeitbudget je Seitenaufruf (`statistics.budget_seconds`, Standard 20 s) und `set_time_limit` im Controller. Nie auf „ein Abruf pro Schuljahr" zurückbauen: ein Jahr sind tausende Bestellungen, ein einziger Abruf läuft minutenlang und speichert bei Abbruch nichts — jeder Versuch begänne wieder bei null. Reicht die Zeit nicht, kommt ein Teilergebnis mit `complete = false` und die Seite bietet „Weiterladen" an.
